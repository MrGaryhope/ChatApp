...
//  Chst
//
//  Created by Егор Максимов on 28.11.2021.
//
...
class ConversationTableViewCell: UITableViewCell {
    ...
    public func configure(with model: Conversation) {
        ...
        StorageManager.shared.downloadURL(for: path, completeon: { [weak self] result in
            switch(result) {
            ...
            case .failure(let error):
                print("Failed to download users image for conversation cell: \(error)")
            }
        })
    }
}
